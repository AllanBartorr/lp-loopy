    ...
    <div className={`p-5 rounded-2xl bg-zinc-100/60 border border-zinc-300 ${SHADOW}`}>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          className={`h-10 w-10 rounded-xl border border-zinc-400 flex items-center justify-center text-xl ${GRADIENT} ${SHADOW} text-zinc-900`}
          aria-label="Diminuir"
        >
          –
        </button>
        ...
        <div className="w-24 text-right text-lg font-semibold text-zinc-900">
          {value}
        </div>
      </div>
    </div>
...
function CheckoutModal({ open, onClose, payload }) {
  ...
  function showSuccessMessage() {
    ...
    messageBox.className = 'fixed inset-0 z-[100] flex items-center justify-center bg-black/70';
    messageBox.innerHTML = `
      <div class="bg-zinc-100 p-8 rounded-xl shadow-2xl border border-zinc-300 text-center max-w-sm">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Content */}
      <div className={`relative w-full max-w-2xl rounded-3xl border border-zinc-400 overflow-hidden ${SHADOW}`}>
        <div className={`px-6 py-4 bg-slate-500`}>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div className="h-0.5 w-6 bg-zinc-900/50 rounded-full"></div>
        ...
        <div className="bg-zinc-100 p-6"> {/* Fundo do modal ajustado */}
          {/* Plano selecionado */}
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              ...
              className={`rounded-2xl px-4 py-3 border transition ${billing === "mensal" ? `${GRADIENT} ${SHADOW} text-zinc-900 border-transparent` : "border-zinc-400 bg-zinc-200/40 text-zinc-800 hover:bg-zinc-200/70"}`}
            >
            ...
            <button
              ...
              className={`rounded-2xl px-4 py-3 border transition ${billing === "anual" ? `${GRADIENT} ${SHADOW} text-zinc-900 border-transparent` : "border-zinc-400 bg-zinc-200/40 text-zinc-800 hover:bg-zinc-200/70"}`}
            >
              {/* Adicionado flex para garantir que o texto "20% OFF" não quebre muito mal */}
              <div className="font-bold flex items-center justify-center">Anual <span className="ml-2 text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-500/90 text-white">20% OFF</span></div>
              {/* Ajuste de tamanho da fonte para os valores */}
          ...
          <div className="mt-8 flex items-center justify-between">
            <button onClick={onClose} className="text-sm px-4 py-2 rounded-xl bg-zinc-300 border border-zinc-400 text-zinc-800 hover:bg-zinc-400 transition">Cancelar</button>
            <div className="flex gap-2">
              {step > 1 && (
                <button onClick={prev} className="text-sm px-4 py-2 rounded-xl bg-zinc-300 border border-zinc-400 text-zinc-800 hover:bg-zinc-400 transition">Voltar</button>
              )}
              {step < 3 ? (
                <button onClick={next} className={`text-sm px-5 py-2.5 rounded-xl font-semibold ${GRADIENT} ${SHADOW} text-zinc-900 hover:opacity-95 transition bg-slate-300 hover:bg-slate-400`}>Próximo</button>
              ) : (
                <button onClick={finish} className={`text-sm px-5 py-2.5 rounded-xl font-semibold ${GRADIENT} ${SHADOW} text-zinc-900 hover:opacity-95 transition`}>Finalizar Contratação</button>
              )}
            </div>
          </div>
        </div>
            ...
            <div className={`rounded-3xl p-5 border border-zinc-400 bg-zinc-100/60 ${SHADOW}`}>
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-sm flex items-center justify-center gap-2">Plano Anual <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-500/90 text-white">20% OFF</span></div>
                  {/* Ajuste de tamanho da fonte para os valores e leading-tight para evitar overflow */}
                  <div className="text-xl font-extrabold mt-1 leading-tight">{formatBRL(annualMonthly)}</div>
                  <div className="text-xs opacity-80 -mt-0.5">por mês</div>
                </button>
